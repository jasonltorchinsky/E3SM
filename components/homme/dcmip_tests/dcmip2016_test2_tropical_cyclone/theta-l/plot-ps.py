...
def main(argv):
    ...
    ps_max = 101920  # Colorbar max [Pa]
    ps0    = 101500  # Reference pressure [Pa]
    ps_min = 101080  # Colorbar min [Pa]
    ps_levs = np.linspace(ps_min, ps_max, 37)
    ps_ticks = np.linspace(ps_min, ps_max, 7, dtype = np.uint)
    days = [5, 10, 15, 20]

    projection = ccrs.PlateCarree(central_longitude = -150)

    # Pressure adjustment for plotting in height coordinate
    # Desn't affect pressure coordinate
    with xr.open_dataset(file_name) as ds:
        ps_offset = ps0 - np.amax(ds.ps.isel(time = 0).to_numpy())
    ...
    for ax_idx in range(nplots):
        ...
        # Extract surface pressure, shift 180 degrees longitude
        ps = ds.ps.isel(time=day) + ps_offset # [Pa]
        nlat, nlon = np.shape(ps.values)
        ps = ps.roll(lon = int(nlon/2))
        
        # Fix the artifact of not-shown-data around 0 and 360-degree longitudes
        ps = gv.xr_add_cyclic_longitudes(ps, "lon")
        
        ax.coastlines(linewidths=0.5)
        ax.add_feature(cfeature.LAND, facecolor="lightgray")
        
        # Contourf-plot data
        cplt = ps.plot.contourf(ax = ax,
    ...
    # Determine title from prefix
    title = r"Tropical Cyclone Surface Pressure Evolution"
