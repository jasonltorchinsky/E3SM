...
subroutine dcmip2012_test2_0(elem,hybrid,hvcoord,nets,nete)
  ...
  do ie = nets,nete; 
     do j=1,np; do i=1,np
        do k=1,nlevp
           call get_coordinates(lat,lon,hyai,hybi,i,j,k,elem(ie),hvcoord,'i')
        ...
        enddo
        do k=1,nlev
           call get_coordinates(lat,lon,hyam,hybm, i,j,k,elem(ie),hvcoord,'m')
...
subroutine dcmip2012_test2_x(elem,hybrid,hvcoord,nets,nete,shear)
  ...
  integer             :: zcoords                                        ! DCMIP version of hcoord
  logical,  parameter :: use_eta = .true.                               ! we are using hybrid eta coords
  real(rl), parameter ::   &
  ...
  if (zcoords == 0) then
     ...
     call abortmp("ERROR: Invalid height coordinate option found in subroutine dcmip2012_test2_x. Aborting...")
  end if
  call set_hybrid_coefficients(hvcoord,hybrid,  hvcoord%etai(1), 1.0_rl)! set hybrid A and B from eta levels
  call set_layer_locations(hvcoord, .true., hybrid%masterthread)
  ...
  do ie = nets,nete; 
     ...
     enddo; enddo
     ! Don't enforce hydrostatic balance for height coord
     if (zcoords == 0) call tests_finalize(elem(ie),hvcoord)
  enddo
...
subroutine dcmip2012_test3(elem,hybrid,hvcoord,nets,nete)
  ...
  do ie = nets,nete
     ...
     enddo; enddo; enddo; 
     ! dont enforce hydrostatic balance for height coord
     if (hcoord==0) call tests_finalize(elem(ie),hvcoord)
  enddo
...
!_____________________________________________________________________
subroutine get_coordinates(lat,lon,hya,hyb,i,j,k,elem,hvcoord,opt)

  ! get lat,lon, vertical coords at node(i,j,k)

  real(rl),         intent(out):: lon,lat,hya,hyb
  integer,          intent(in) :: i,j,k
  type(element_t),  intent(in) :: elem
  type(hvcoord_t),  intent(in) :: hvcoord
  character(len=1), intent(in) :: opt ! Get interface values (i) or 
  ! midpoint values (m)
  ...
  ! get hybrid coefficients at vertical level k
  if (opt .eq. 'i') then
     hya = hvcoord%hyai(k)
     hyb = hvcoord%hybi(k)
     ...
     hya = hvcoord%hyam(k)
     hyb = hvcoord%hybm(k)
  else ! IN CASE OF ERROR, ABORT
     call abortmp('ERROR: Invalid option for get_coordinates in dcmip12_wrapper.F90.')
  end if
