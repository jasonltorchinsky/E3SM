...
xr_var_attrs = {"T" : {"units" : "K",
    ...
    "T_eddy" : {"units" : "K^2",
               "description" : "Eddy Variance of Temperature"},
}
...
plt_var_attrs = {"T" : {"label" : r"Temperature $\left[ K \right]$",
    ...
    "w" : {"label" : r"Vertical Wind Velocity $\left[ m\,s^{-1} \right]$",
           "cmap" : "RdBu"},
    "T_eddy" : {"label" : r"Eddy Variance of Temperature $\left[ K^{2} \right]$",
           "cmap" : "plasma"},
}
    ...
    for plot_var in plot_vars:
        ...
        if recalculate or (not os.path.exists(clim_filepath)):
            ...
            with (xr.open_dataset(homme_output, engine = "netcdf4", decode_timedelta = False)
                ...
                if l_rank == MPI_ROOT:
                    datetime_now = datetime.now().strftime("%H:%M:%S")
                    msg = "[{}]: Creating target pressure grid.".format(datetime_now)
                    print(msg, flush = True)
                ...
                if l_rank == MPI_ROOT:
                    datetime_now = datetime.now().strftime("%H:%M:%S")
                    msg = "[{}]: Vertically remapping field to target pressure grid.".format(datetime_now)
                    print(msg, flush = True)
            ...
            if l_rank == MPI_ROOT:
                datetime_now = datetime.now().strftime("%H:%M:%S")
                msg = "[{}]: Calculating climatology.".format(datetime_now)
                print(msg, flush = True)
                
            g_field_clim = calc_clim(l_field_vremap, plot_var, comm)
...
def vremap_field(homme_ds, p_tgt, plot_var, comm):
    supported_plot_vars = ["T", "u", "v", "w", "T_eddy"]
    ...
    #---------------------------------------------------------------------------
    if "eddy" in plot_var:
        field_key = plot_var[0]
    elif plot_var in ["T", "u", "v", "w"]:
        field_key = plot_var
...
def calc_clim(l_field_vremap, plot_var, comm):
    supported_plot_vars = ["T", "u", "v", "w", "T_eddy"]
    assert(plot_var in supported_plot_vars)
    ...
    #---------------------------------------------------------------------------
    if "eddy" in plot_var:
        l_field_zonal_mean = np.sum(l_field_vremap, axis = (3), where = ~np.isnan(l_field_vremap))[...,None] # [l_nt, nz, nlat, 1]
        l_field_sum = np.sum(np.pow(l_field_vremap - l_field_zonal_mean, 2), axis = (0, 3), where = ~np.isnan(l_field_vremap)) # [nz, nlat]
    else:
...
def plot_clim(plot_var, clim, clim_fileroot, tag, plotting_dir, set_ylim = True):
    ...
    clim_plt = axs.pcolormesh(lat, sigma, clim,
        vmin = vmin, vmax = vmax, cmap = cmap)
    axs.axvline([0], color = "grey")
    axs.axhline([pk02_p_sp / ps], color = "grey", linestyle = "dashed", linewidth = 2.0,
        label = "PK 2002 Sponge Layer Height")
    axs.axhline([pk02_p_T / ps], color = "grey", linewidth = 2.0,
    ...
    if zero_levels is not None:
        # Zero contour
        axs.contour(lat, sigma, clim, levels = zero_levels,
    ...
    if pos_levels is not None:
        # Positive contours
        axs.contour(lat, sigma, clim, levels = pos_levels,
    ...
    if neg_levels is not None:
        # Negative contours
        axs.contour(lat, sigma, clim, levels = neg_levels,
            colors = line_color, linewidths = nonzero_linewidth,
